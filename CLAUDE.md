...
This document provides guidance for working effectively in the Bluesky Social
app codebase.

## Project Overview

Bluesky Social is a cross-platform social media application built with React
Native and Expo. It runs on iOS, Android, and Web, connecting to the AT Protocol
(atproto) decentralized social network.
...
## Bluesky on Crack Notes

- All crack-related libraries, hooks, and components must live under a `crack/`
  subdirectory within their parent folder.
- Alter Ego records use the `dev.ocbwoy3.crack.alterego` lexicon and must be
  client-validated before use.
- Alter Ego is a display-only overlay; it must never mutate underlying
  session/auth state.
- Prefer hijacking the ATProto client if necessary instead of modifying many UI
  components.
...
ALF is the custom design system. It uses Tailwind-inspired naming with
...
Dialogs use a bottom sheet on native and a modal on web. Use
`useDialogControl()` hook to manage state.
...
- `color`: `'primary'` | `'secondary'` | `'negative'` | `'primary_subtle'` |
  `'negative_subtle'` | `'secondary_inverted'`
...
function MyComponent() {
  ...
  return (
    <Text>
      <Trans>
        Welcome to <Text style={a.font_bold}>Bluesky</Text>
      </Trans>
    </Text>
  );
}
...
**Paginated APIs:** Many atproto APIs return paginated results with a `cursor`.
Use `useInfiniteQuery` for these:
...
**Important:** The bundler automatically resolves platform-specific files. Just
import normally:
...
**Always use `control.close(() => ...)` when performing actions after closing a
dialog.** The callback ensures the action runs after the dialog's close
animation completes. Failing to do this causes race conditions with React state
updates.
...
The Menu component on iOS specifically uses this pattern - see
`src/components/Menu/index.tsx:151`.
...
This codebase uses React Compiler, so **don't proactively add `useMemo` or
`useCallback`**. The compiler handles memoization automatically.
...
3. **Styling**: Combine static atoms with theme atoms, use platform utilities
   for platform-specific styles

4. **State**: Use TanStack Query for server state, React Context for UI
   preferences

5. **Components**: Check if a component exists in `#/components/` before
   creating new ones

6. **Types**: Define explicit types for props, use `NativeStackScreenProps` for
   screens
